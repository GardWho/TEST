  ...
  return (
    <div className="bg-[#F5EFE4] min-h-screen">
      ...
      <div className="max-w-[1400px] mx-auto px-8 md:px-14 py-12 space-y-12">
        ...
        {/* ============================================================ */}
        {/* SECTION : COURS PARTICULIERS (AGRANDIE) */}
        {/* ============================================================ */}
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div>
            ...
            {/* ✅ CARTES AGRANDIES (min-h-[220px] + p-6) */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {coursParticuliers.map((item) => (
                <div
                  key={item.label}
                  className="bg-[#EDE4D0] p-6 rounded-sm hover:bg-[#E8DDD0] transition-colors text-center flex flex-col min-h-[220px]"
                >
                  <div className="flex-1 flex flex-col justify-center">
                    <h3 className="text-base font-normal text-[#1C1814] mb-1" style={{ fontFamily: "'Playfair Display', serif" }}>
                      {item.label}
                    </h3>
                    <p className="text-2xl font-medium text-[#C09A3C] mb-2">{item.price} €</p>
                  </div>
                  <button
                    onClick={() => addItem(item.label, item.price, "cours")}
                    className="w-full py-2 text-[10px] tracking-[0.2em] uppercase bg-[#C09A3C] text-white hover:bg-[#1C1814] transition-colors mt-auto"
                  >
                    RÉSERVER
                  </button>
                </div>
        ...
        {/* ============================================================ */}
        {/* SECTION : COURS COLLECTIFS (AGRANDIE) */}
        {/* ============================================================ */}
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
            {/* Gauche : 3 paliers (AGRANDIS) */}
            <div className="space-y-3 flex flex-col justify-center">
              {paliers.map((palier) => (
                <div
                  key={palier.label}
                  className="bg-[#EDE4D0] p-5 md:p-6 rounded-sm text-center hover:bg-[#E8DDD0] transition-colors"
                >
                  <span className="text-[15px] text-[#1C1814] font-light block">{palier.label}</span>
                  <span className="text-[20px] font-medium text-[#C09A3C]">{palier.price}€/personne</span>
                </div>
              ))}
            </div>

            {/* Droite : Sélecteur (AGRANDI) */}
            <div className="bg-[#EDE4D0] p-6 md:p-8 rounded-sm flex flex-col items-center justify-center h-full">
      ...
      <div className="max-w-[1400px] mx-auto px-8 md:px-14 py-12 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-[#C09A3C]/15">
          <div className="bg-[#EDE4D0] p-6 rounded-sm hover:bg-[#E8DDD0] transition-colors">
            <h3 className="text-xl font-normal text-[#1C1814] mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>
              Examens fédéraux
            </h3>
            ...
            <Link to="/contact" className="inline-block mt-3 px-5 py-2 text-[9px] tracking-[0.2em] uppercase bg-[#C09A3C] text-white hover:bg-[#1C1814] transition-colors">
              Me contacter
            </Link>
          </div>
          <div className="bg-[#EDE4D0] p-6 rounded-sm hover:bg-[#E8DDD0] transition-colors">
