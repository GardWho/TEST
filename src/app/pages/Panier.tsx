        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-sm shadow-sm sticky top-24">
              ...
              <div className="mb-4">
                <label className="text-[10px] tracking-[0.3em] uppercase text-[#1C1814]/40 block mb-2">Lieu d'intervention</label>
                <input type="number" value={deliveryKm} onChange={(e) => setDeliveryKm(Number(e.target.value))} placeholder="Kilomètres (au-delà de 15 km)" className="w-full border border-[#C09A3C]/10 rounded-sm px-3 py-2 text-[14px] outline-none focus:border-[#C09A3C] transition-colors bg-[#F8F3EC]" min="0" />
                {deliveryKm > 15 && <p className="text-[11px] text-[#C09A3C] mt-1">+{((deliveryKm - 15) * 0.50).toFixed(2)} € de frais de déplacement</p>}
              </div>
              <div className="space-y-2 pt-4 border-t border-[#C09A3C]/10">
                <div className="flex justify-between text-[14px] text-[#1C1814]/60"><span>Sous-total</span><span>{total.toFixed(2)} €</span></div>
                {deliveryKm > 15 && <div className="flex justify-between text-[14px] text-[#C09A3C]"><span>Frais de déplacement</span><span>{((deliveryKm - 15) * 0.50).toFixed(2)} €</span></div>}
              ...
              <button onClick={proceedToCheckout} disabled={loading} className="w-full mt-6 py-3 bg-[#1C1814] text-white text-[11px] tracking-[0.25em] uppercase hover:bg-[#C09A3C] transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? "Traitement..." : `Payer ${totalWithDelivery.toFixed(2)} €`}
              </button>
              <p className="text-[10px] text-[#1C1814]/25 text-center mt-3 tracking-wider">Paiement sécurisé Stripe</p>
            </div>
          </div>
        </div>
      </div>
